...
contract ShieldedPool is IShieldedPool, ReentrancyGuard {
    ...
    event InternalMatchSettled(
        ...
        bytes32 takerOutputCommitmentSwap,
        bytes32 takerOutputCommitmentChange,
        bytes32 makerOutputCommitmentSwap,
        bytes32 makerOutputCommitmentChange,
        uint256 takerProtocolFee,
        uint256 makerProtocolFee,
        uint256 takerGasRefund,
        uint256 makerGasRefund,
        address relayer
    );
    ...
    /**
     * @notice Atomically settle a deterministic off-chain internal match.
     * @dev Additive entrypoint. Preserves existing swap/withdraw flows unchanged.
     */
    function internalMatchSettle(
        InternalMatchSettlementData calldata data
    ) external override nonReentrant onlyRelayer {
        JoinSplitPublicInputs memory taker = data.takerInputs;
        JoinSplitPublicInputs memory maker = data.makerInputs;

        // Both legs must bind to the same pre-state root and be live at execution time.
        if (taker.merkleRoot != maker.merkleRoot || taker.merkleRoot != merkleRoot) revert PoolErr(46);

        // Nullifier one-time spend protection.
        if (nullifiers[taker.nullifier] || nullifiers[maker.nullifier]) revert PoolErr(4);
        if (taker.nullifier == maker.nullifier) revert PoolErr(47);

        // Validator consensus + proof validity for both legs.
        if (!thresholdVerifier.verifyProof(data.takerProof, _joinSplitPublicInputsToArray(taker))) revert PoolErr(40);
        if (!thresholdVerifier.verifyProof(data.makerProof, _joinSplitPublicInputsToArray(maker))) revert PoolErr(40);
        if (!_verifyJoinSplitProof(data.takerProof, taker) || !_verifyJoinSplitProof(data.makerProof, maker)) revert PoolErr(6);

        // Witness/leaf inclusion checks on the same root snapshot.
        if (!MerkleTree.verifyProof(
                taker.inputCommitment,
                taker.merkleRoot,
                _convertToBytes32Array(taker.merklePath),
                taker.merklePathIndices,
                MAX_TREE_DEPTH
            )) revert PoolErr(42);
        if (!MerkleTree.verifyProof(
                maker.inputCommitment,
                maker.merkleRoot,
                _convertToBytes32Array(maker.merklePath),
                maker.merklePathIndices,
                MAX_TREE_DEPTH
            )) revert PoolErr(42);

        // Internal match never routes to DEX leg directly.
        if (taker.outputAssetIDChange != taker.inputAssetID || maker.outputAssetIDChange != maker.inputAssetID) revert PoolErr(45);
        if (taker.swapAmount == 0 || maker.swapAmount == 0) revert PoolErr(20);
        if (taker.changeAmount == 0 || maker.changeAmount == 0) revert PoolErr(19);
        if (taker.outputAmountSwap < taker.minOutputAmountSwap || maker.outputAmountSwap < maker.minOutputAmountSwap) revert PoolErr(15);

        // Cross-leg asset/amount compatibility.
        if (
            taker.inputAssetID != maker.outputAssetIDSwap ||
            maker.inputAssetID != taker.outputAssetIDSwap
        ) revert PoolErr(47);
        if (
            taker.swapAmount != maker.outputAmountSwap ||
            maker.swapAmount != taker.outputAmountSwap
        ) revert PoolErr(47);

        // Per-leg conservation + fee integrity checks.
        uint256 takerTotal = taker.swapAmount + taker.changeAmount + taker.protocolFee + taker.gasRefund;
        uint256 makerTotal = maker.swapAmount + maker.changeAmount + maker.protocolFee + maker.gasRefund;
        if (taker.inputAmount != takerTotal || maker.inputAmount != makerTotal) revert PoolErr(43);

        address takerInputToken = taker.inputAssetID == 0 ? address(0) : assetRegistry[taker.inputAssetID];
        address makerInputToken = maker.inputAssetID == 0 ? address(0) : assetRegistry[maker.inputAssetID];
        if ((takerInputToken == address(0) && taker.inputAssetID != 0) || (makerInputToken == address(0) && maker.inputAssetID != 0)) revert PoolErr(11);

        feeOracle.requireFreshPrice(takerInputToken);
        feeOracle.requireFreshPrice(makerInputToken);

        uint256 takerExpectedFee = feeOracle.calculateFee(takerInputToken, taker.inputAmount) + _calculateSwapFee(taker.inputAmount);
        uint256 makerExpectedFee = feeOracle.calculateFee(makerInputToken, maker.inputAmount) + _calculateSwapFee(maker.inputAmount);
        if (taker.protocolFee != takerExpectedFee || maker.protocolFee != makerExpectedFee) revert PoolErr(5);
        if (taker.gasRefund > taker.inputAmount || maker.gasRefund > maker.inputAmount) revert PoolErr(7);

        // Atomic state transitions: insert all outputs, then mark both nullifiers.
        (uint256 takerSwapIndex, bytes32 root1) = tree.insert(taker.outputCommitmentSwap);
        commitments[takerSwapIndex] = taker.outputCommitmentSwap;
        merkleRoot = root1;
        commitmentCount = tree.nextIndex;

        (uint256 takerChangeIndex, bytes32 root2) = tree.insert(taker.outputCommitmentChange);
        commitments[takerChangeIndex] = taker.outputCommitmentChange;
        merkleRoot = root2;
        commitmentCount = tree.nextIndex;

        (uint256 makerSwapIndex, bytes32 root3) = tree.insert(maker.outputCommitmentSwap);
        commitments[makerSwapIndex] = maker.outputCommitmentSwap;
        merkleRoot = root3;
        commitmentCount = tree.nextIndex;

        (uint256 makerChangeIndex, bytes32 root4) = tree.insert(maker.outputCommitmentChange);
        commitments[makerChangeIndex] = maker.outputCommitmentChange;
        merkleRoot = root4;
        commitmentCount = tree.nextIndex;

        nullifiers[taker.nullifier] = true;
        nullifiers[maker.nullifier] = true;

        address relayer = data.relayer != address(0) ? data.relayer : msg.sender;

        // Distribute per-leg protocol fees in each input asset domain.
        if (takerExpectedFee > 0) {
            if (takerInputToken == address(0)) {
                IFeeDistributor(address(relayerRegistry)).distributeFee{value: takerExpectedFee}(address(0), takerExpectedFee);
            } else {
                IERC20(takerInputToken).approve(address(relayerRegistry), takerExpectedFee);
                IFeeDistributor(address(relayerRegistry)).distributeFee(takerInputToken, takerExpectedFee);
            }
        }
        if (makerExpectedFee > 0) {
            if (makerInputToken == address(0)) {
                IFeeDistributor(address(relayerRegistry)).distributeFee{value: makerExpectedFee}(address(0), makerExpectedFee);
            } else {
                IERC20(makerInputToken).approve(address(relayerRegistry), makerExpectedFee);
                IFeeDistributor(address(relayerRegistry)).distributeFee(makerInputToken, makerExpectedFee);
            }
        }

        // Refund relayer from each leg if requested.
        if (taker.gasRefund > 0) {
            if (takerInputToken == address(0)) {
                payable(relayer).transfer(taker.gasRefund);
            } else {
                IERC20(takerInputToken).transfer(relayer, taker.gasRefund);
            }
        }
        if (maker.gasRefund > 0) {
            if (makerInputToken == address(0)) {
                payable(relayer).transfer(maker.gasRefund);
            } else {
                IERC20(makerInputToken).transfer(relayer, maker.gasRefund);
            }
        }
        ...
        emit InternalMatchSettled(
            ...
            taker.nullifier,
            maker.nullifier,
            taker.outputCommitmentSwap,
            taker.outputCommitmentChange,
            maker.outputCommitmentSwap,
            maker.outputCommitmentChange,
            takerExpectedFee,
            makerExpectedFee,
            taker.gasRefund,
            maker.gasRefund,
        ...
        emit NullifierMarked(taker.nullifier);
        emit NullifierMarked(maker.nullifier);
        emit CommitmentAdded(taker.outputCommitmentSwap, takerSwapIndex);
        emit CommitmentAdded(taker.outputCommitmentChange, takerChangeIndex);
        emit CommitmentAdded(maker.outputCommitmentSwap, makerSwapIndex);
        emit CommitmentAdded(maker.outputCommitmentChange, makerChangeIndex);

        if (data.encryptedPayload.length > 0) {
            emit EncryptedPayload(taker.nullifier, data.encryptedPayload);
        }
    }
