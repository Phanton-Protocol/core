...
contract ShieldedPoolUpgradeableReduced is IShieldedPool, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // ============ M3 legacy slots (Path B: deprecated, layout preserved) ============
    // **Storage-layout safety:** Path B removes `internalMatchSettle` but keeps
    // these four mappings declared (renamed `__deprecated…`) so any proxy that
    // already received the M3 UUPS upgrade retains its slot layout. New deploys
    // simply never touch these slots. M6 (Path B enrollment registry) appends
    // new state AFTER this block.
    mapping(bytes32 => bool) internal __deprecatedUsedInternalMatchHashes;
    mapping(bytes32 => bool) internal __deprecatedUsedInternalDecisionHashes;
